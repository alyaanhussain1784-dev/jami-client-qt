...
def get_total_ram_gb():
    """Return total physical RAM in Gigabytes, or None if undetermined."""
    try:
...
def get_default_jobs(ram_per_job_gb=2.0):
    """Calculate safe default parallel jobs based on CPU cores and total RAM.
    Capping jobs so each build process has ~2 GB RAM prevents system OOM/swapping."""
    cpus = multiprocessing.cpu_count()
    ram_gb = get_total_ram_gb()
    if ram_gb is not None and ram_gb > 0:
        ram_jobs = max(1, int(ram_gb // ram_per_job_gb))
        return max(1, min(cpus, ram_jobs))
    return max(1, cpus)
...
def parse_args():
    ...
    ap.add_argument('-j', '-p', '--jobs', type=int, default=None,
                    help='Number of parallel build jobs (default: auto-detected based on CPU cores and RAM)')
